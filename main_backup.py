...
import lcddriver #install this from https://github.com/the-raspberry-pi-guy/lcd
...
PIC_RES_Y = 
#File locations
HIGH_TEMP_LOCATION = # location to save high temp ppl pics
MULTIPLE_PPL_LOCATION = # location to save pics of ppl not following socical distancing
NO_MASK_LOCATION = # location to save pics of ppl who dont wear mask
NO_FACE_LOCATION = # location to save images when sensor was triggered but no face detected

#PINS
P_ON_OFF = 1  #trigger pin - this will be middle button of joystick
P_MOTION_DETECTION = #trigger pin - motion detector sensor
P_SERVO_X = 
P_SERVO_Y = 
P_LAZER = 
P_DOOR = # should be connected to relay
...
def sensorTriggeredButNoFace(frame):
    ...
    LogP("I thought someone was here.. not i can see no one")
    cv2.imwrite(NO_FACE_LOCATION, frame)


def noMask(frame):
    #this will be executed when person has no mask
    LogP("NO mask detected, image will be saved at {}".format())
    cv2.imwrite(NO_MASK_LOCATION, frame)

def multipleFaces(frame):
    # this will be executed when multiple faces were found
    LogP("Multiple faces found, image will be saved at {}".format(MULTIPLE_PPL_LOCATION))
    cv2.imwrite(MULTIPLE_PPL_LOCATION, frame)


def highTemp(frame):
    # this will be executed when a person with high temp is dtected
    LogP("HIGH TEMP SCANNED, image will be saved at {}".format(HIGH_TEMP_LOCATION))
    cv2.imwrite(HIGH_TEMP_LOCATION, frame)
...
def DisplayVideoOutput(locs, preds, frame, mask,withoutMask):
    if not DEBUG:
        return
